...
#For this task, you will be given a completed 3x3 square, in the form of a two-dimensional list.
# Create a function that checks to make sure this 3x3 square contains each number from 1-9 exactly once. Make sure there are no duplicates, and no numbers outside this range.
...
def is_mini_sudoku(square):
    numL = [i for x in square for i in x]
    return sorted(numL) == [1,2,3,4,5,6,7,8,9]
